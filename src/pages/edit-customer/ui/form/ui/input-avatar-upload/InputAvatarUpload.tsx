...
interface InputAvatarUploadProps {
  isPending: boolean;
  errors?: string[];
  value: Uint8Array | null;
}

export const InputAvatarUpload = ({ value, isPending, errors }: InputAvatarUploadProps) => {
  // Errors
  const [sizeError, setSizeError] = useState<string | null>(null);
  const [typeError, setTypeError] = useState<string | null>(null);

  // Preview
  const initialPreview = value ? URL.createObjectURL(new Blob([value], { type: 'image/png' })) : null;
  const [imageSrcPreview, setImageSrcPreview] = useState<string | null>(initialPreview);
